...
#from preact_resnetv3 import PreActResNet50
from backbone.preact_resnet50 import PreActResNet50
import cv2
import numpy as np
...
class tinyquadra_multi_resUnet_ly_masks(nn.Module):
    def __init__(self, in_ch, out_ch1, out_ch2, out_ch3, multitask=False, device='cuda', resize=None):
        ...
        self.conv9_m = singleConv(32, 8, 3, act=True)
        self.conv9_mm = singleConv(32 + 8 * (out_ch1 - 1), 32, 1, act=True)
        self.conv8_eca = eca_layer(64)
        self.conv8_m = singleConv(64, 16, 3, act=True)
        self.conv8_mm = singleConv(64 + 16 * (out_ch1 - 1), 64, 1, act=True)
        self.conv7_eca = eca_layer(128)
        self.conv7_m = singleConv(128, 32, 3, act=True)
        self.conv7_mm = singleConv(128 + 32 * (out_ch1 - 1), 128, 1, act=True)
        self.conv6_eca = eca_layer(256)
        self.conv6_m = singleConv(256, 64, 3, act=True)
        self.conv6_mm = singleConv(256 + 64 * (out_ch1 - 1), 256, 1, act=True)
        self.conv5_eca = eca_layer(512)
        self.conv5_m = singleConv(512, 128, 3, act=True)
        self.conv5_mm = singleConv(512 + 128 * (out_ch1 - 1), 512, 1, act=True)
        self.conv1_1 = singleConv(in_ch, 32, 3)
        self.conv1_2 = singleConv(1, 32, 3)
        self.conv1_3 = singleConv(32, 32, 3)
        ...
        self.AM8 = AM(96, 64)
        self.conv10 = nn.Sequential(Conv2d(32, out_ch1, 1))
        self.conv10_1 = nn.Sequential(Conv2d(64, out_ch1, 1))
        self.conv11 = nn.Sequential(Conv2d(64, out_ch2, 1))
        self.conv12 = Conv2d(32, out_ch3, 1)
    ...
    def forward(self, x):
        ...
        c1 = self.pool1(c1)
        #c1 = checkpoint.checkpoint(self.trans1, c1)
        #c2 = self.conv2(c1)
        c2 = self.conv2(c1)
        adp_conv2 = self.adp_conv2(c2)
        #c2 = checkpoint.checkpoint(self.trans2, c2)
        #c3 = self.conv3(c2)
        c3 = self.conv3(c2)
        adp_conv3 = self.adp_conv3(c3)
        #c3 = self.trans3(c3)
        #c3 = checkpoint.checkpoint(self.trans3, c3)
        #c4 = self.conv4(c3)
        c4 = self.conv4(c3)
        adp_conv4 = self.adp_conv4(c4)
        #c4 = self.trans4(c4)
        #c4 = checkpoint.checkpoint(self.trans4, c4)
        #c5 = self.conv5(c4)
        c5 = self.conv5(c4)
        #c5 = self.trans5(c5)
        #c5 = checkpoint.checkpoint(self.trans5, c5)
        c5 = self.drop(c5)
        ...
        mask_irf = torch.unsqueeze(masks[:, 0, :, :] + masks[:, 1, :, :] + masks[:, 2, :, :]
                                   + masks[:, 3, :, :] + masks[:, 4, :, :] + masks[:, 5, :, :], dim=1)
        #BM_ILM_mask, act = softargmax2d_col(c10, eta=1, dim=2, device=self.device, add=False)
        # print(BM_ILM_mask)
        # att
        '''BM_ILM_mask = self.conv1_2(BM_ILM_mask).relu()
        BM_ILM_mask = self.spatt(BM_ILM_mask)

        c1_1 = torch.mul(c1, BM_ILM_mask)
        c1_1 = self.conv1_3(c1_1).relu()'''
        # cat
        '''BM_ILM_mask = self.conv1_2(BM_ILM_mask).relu()
        c1_1= torch.cat([c1, BM_ILM_mask], dim=1)'''
        # mul
        if act:
            '''c9 = torch.mul(c9, BM_ILM_mask)
            BM_ILM_mask = self.pool1(BM_ILM_mask)
            self.featuremap1 = BM_ILM_mask

            c8 = torch.mul(c8, BM_ILM_mask)
            BM_ILM_mask = self.pool2(BM_ILM_mask)

            c7 = torch.mul(c7, BM_ILM_mask)
            BM_ILM_mask = self.pool3(BM_ILM_mask)

            c6 = torch.mul(c6, BM_ILM_mask)
            BM_ILM_mask = self.pool4(BM_ILM_mask)
            c5 = torch.mul(c5, BM_ILM_mask)'''
            c9_ = c9
            c8_ = c8
            c7_ = c7
            c6_ = c6
            ...
            for i in range(c10.shape[1] - 1):
                ...
                self.featuremap1 = torch.mul(c9, mask_1)
        ...
        up_8 = self.up8(a3)
        a4 = self.AM4(up_8, c9)
        a4_1 = self.AM4_1(torch.mul(up_8, mask_irf))
        up_9 = self.up9(c5)
        a5 = self.AM5(up_9, a1)
        up_10 = self.up10(a5)
