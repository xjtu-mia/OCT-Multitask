...
from backbone.swin_transformerv2 import SwinTransformerV2
import math
...
class tinyquadra_multi_resUnet_ly_masks_s3(nn.Module):
    def __init__(self, in_ch, out_ch1, out_ch2, out_ch3, multitask=False, device='cuda', resize=(384,512), branch_num=4):
        ...
        encoder = SwinTransformerV2(img_size=resize, window_size=8,in_chans=1)
        '''pretext_model = torch.load('/home/wanghualin/python_project/OCT/OCT_fluid_layer_seg/pytorch_ver/backbone/swinv2_tiny_patch4_window8_256.pth')
        ...
        self.conv0 = singleConv(in_ch, 32, 3, act=True)
        self.conv1 = encoder.patch_embed
        self.pool1 = nn.MaxPool2d(2)
        #self.pool1_ = encoder.to_token1
        #self.pool1_ = singleConv(32, 96, 1, stride=2, act=True)
        #self.conv2 = encoder.layer1
        self.adp_conv1 = upConv(96, 32, 2)
        self.conv2 = encoder.layers[0].to(self.device)
        self.adp_conv2 = upConv(192, 64, 2)
        #self.conv3 = encoder.layer2

        self.conv3 = encoder.layers[1].to(self.device)
        self.adp_conv3 = upConv(384, 128, 2)
        #self.conv4 = encoder.layer3
        self.conv4 = encoder.layers[2].to(self.device)
        self.adp_conv4 = upConv(768, 256, 2)
        #self.conv5 = encoder.layer4
        self.conv5 = encoder.layers[3].to(self.device)
    ...
    def forward(self, x, act=True):
        ...
        adp_conv1 = self.adp_conv1(torch.reshape(c1.transpose(1, 2), [b,c1.size(2),h//2,w//2]))
        #c1 = self.pool1(c1)
        # c1 = checkpoint.checkpoint(self.self.trans1, c1)
        # print(c1.shape)
        c2 = checkpoint.checkpoint(self.conv2, c1)
        # print(c2.shape)
        adp_conv2 = self.adp_conv2(torch.reshape(c2.transpose(1, 2), [b,c2.size(2),h//4,w//4]))
        # c2 = self.trans2(c2)
        c3 = checkpoint.checkpoint(self.conv3, c2)
        adp_conv3 = self.adp_conv3(torch.reshape(c3.transpose(1, 2), [b,c3.size(2),h//8,w//8]))
        # c3 = self.trans3(c3)
        c4 = checkpoint.checkpoint(self.conv4, c3)
        adp_conv4 = self.adp_conv4(torch.reshape(c4.transpose(1, 2), [b,c4.size(2),h//16,w//16]))
        # c4 = self.trans4(c4)
        c5 = checkpoint.checkpoint(self.conv5, c4)
        # c5 = self.trans5(c5)
        c5 = self.drop(c5)
        print(c5.shape)
        adp_conv5 = self.adp_conv5(torch.reshape(c5.transpose(1, 2), [b,c5.size(2),h//16,w//16]))
