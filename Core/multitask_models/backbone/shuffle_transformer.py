...
class PatchEmbedding(nn.Module):
    def __init__(self, inter_channel=32, out_channels=48, in_ch = 32):
        super().__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_ch, inter_channel, kernel_size=3, stride=1, padding=1),
...
class ShuffleTransformer(nn.Module):
    def __init__(self, img_size=(448,448), in_chans=1, num_classes=1000, token_dim=32, embed_dim=96, mlp_ratio=4.,
                 layers=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 relative_pos_embedding=True, shuffle=True, window_size=14, qkv_bias=False, qk_scale=None, drop_rate=0.,
        ...
        self.to_token1 = PatchEmbedding(inter_channel=token_dim, out_channels=embed_dim, in_ch=in_chans)

        num_patches = (img_size[0] * img_size[1]) // 16
