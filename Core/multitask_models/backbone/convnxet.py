...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, size=None):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim)  # depthwise conv
        self.norm = LayerNorm(dim, eps=1e-6)
        self.pwconv1 = nn.Linear(dim, 4 * dim)  # pointwise/1x1 convs, implemented with linear layers
        self.act = nn.GELU()
        #self.act = MetaAconC(size[0])
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
        ...
        stem = nn.Sequential(
            ...
            #Conv2d(in_chans, dims[0], kernel_size=2, stride=1),
            #nn.MaxPool2d(2),
        ...
        for i in range(3):
            downsample_layer = nn.Sequential(
                ...
                #Conv2d(dims[i], dims[i + 1], kernel_size=2, stride=1),
                #nn.MaxPool2d(2),
            )
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[Block(dim=dims[i], drop_path=dp_rates[cur + j],
                        layer_scale_init_value=layer_scale_init_value,size=(208//(2**i), 256//(2**i))) for j in range(depths[i])]
            )
