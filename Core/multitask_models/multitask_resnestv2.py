...
from backbone.preact_resnetv2 import PreActResNet50, co_PreActResNet50, MSCA_PreActResNet50
import cv2
import numpy as np
...
class GaussianBlurConv(nn.Module):
    def __init__(self, channels=3, bs = 2):
        ...
        self.conv = nn.Conv2d(channels, channels, 5, groups=channels, bias=False, padding=2)
...
class Swish(nn.Module):
    ...
    # AconA: x * torch.sigmoid(self.beta * x), beta is a learnable parameter
...
'''def sobel_conv2d(im, inch, outch,divece=torch.device('cuda')):
    # 用nn.Conv2d定义卷积操作
    b,c,h,w=im.shape[0],im.shape[1],im.shape[2],im.shape[3]
    conv_op = Conv2d(c, c, 3, bias=False)
    # 定义sobel算子参数
    sobel_kernel = np.array([[[[1, 2, 1], [0, 0, 0], [-1, -2, -1]]*outch]*b], dtype='float32')
    # 将sobel算子转换为适配卷积操作的卷积核
    sobel_kernel = sobel_kernel.reshape((-1, c, 3, 3))
    # 给卷积操作的卷积核赋值
    conv_op.weight.data = torch.from_numpy(sobel_kernel).to(divece)
    # 对图像进行卷积操作
    print(conv_op.weight.shape)
    edge_detect = conv_op(im)
    print(edge_detect.shape)
    # 将输出转换为图片格式
    #edge_detect = edge_detect.squeeze().detach().numpy()
    return edge_detect'''
'''class sobel_conv2d(nn.Module):
    def __init__(self, inch, outch,divece=torch.device('cuda')):
        super(sobel_conv2d, self).__init__()
    ...
    def forward(self, input):
        ...
        sobel_kernel = torch.FloatTensor([[[[1, 2, 1], [0, 0, 0], [-1, -2, -1]] * c] * b])
        weight = nn.Parameter(data=sobel_kernel, requires_grad=False)
        # 将sobel算子转换为适配卷积操作的卷积核
        sobel_kernel = sobel_kernel.reshape((b, c, 3, 3))

        # 给卷积操作的卷积核赋值

        return F.conv2d(input, weight, padding=1)'''
class tinyquadra_multi_resUnet_ly_masks(nn.Module):
    def __init__(self, in_ch, out_ch1, out_ch2, out_ch3, multitask=True, device='cuda', resize=None, branch_num=4):
        super(tinyquadra_multi_resUnet_ly_masks, self).__init__()
        ...
        encoder = PreActResNet50(in_ch=in_ch)
        ...
        self.gaussian_conv_1 = GaussianBlurConv(6)
        self.gaussian_conv_2 = GaussianBlurConv(1)
    ...
    def forward(self, x, act=True):
        ...
        if self.multitask:
            ...
            mask_irf = torch.sum(masks, dim=1, keepdim=True)
            '''masks = self.gaussian_conv_1(masks)
            mask_irf = self.gaussian_conv_2(mask_irf)'''
            self.featuremap1 = mask_irf
            ...
            a4 = self.AM4(up_8, c9)
            a4_1 = self.AM4_1(torch.mul(up_8, mask_irf) if act else up_8)
            ...
            if self.resize:
                c10_1 = F.interpolate(c10_1, (h1, w1), mode='bilinear')
                c10 = F.interpolate(c10, (h1, w1), mode='bilinear')
                c12 = F.interpolate(c12, (h1, w1), mode='bilinear')
                c13 = F.interpolate(c13, (h1, w1), mode='bilinear')
            if self.branch_num == 4:
...
class tinyquadra_multi_resUnet_ly_masks_s4(nn.Module):
    def __init__(self, in_ch, out_ch1, out_ch2, out_ch3, multitask=True, device='cuda', resize=None, branch_num=4):
        super(tinyquadra_multi_resUnet_ly_masks_s4, self).__init__()
        self.multitask = multitask
        self.device = device
        self.resize = resize
        self.branch_num = branch_num
    ...
    def forward(self, x, act=True):
        ...
        if self.multitask:
            ...
            #up_8_1 = self.up8_1(a3)
            a4 = self.AM4(up_8, c9)
            a4_1 = self.AM4_1(torch.mul(a4, mask_irf) if act else a4)
            #a4_1 = self.AM4_1(torch.mul(torch.cat([up_8_1, adp_conv1], dim=1), mask_irf) if act else torch.cat(up_8, adp_conv1))
            if self.branch_num == 4:
                up_9 = self.up9(c5)
                a5 = self.AM5(up_9, a1)
...
class tinyquadra_multi_resUnet_ly_masks_s2(nn.Module):
    def __init__(self, in_ch, out_ch1, out_ch2, out_ch3, multitask=True, device='cuda', resize=None, branch_num=4):
        super(tinyquadra_multi_resUnet_ly_masks_s2, self).__init__()
        self.multitask = multitask
        self.device = device
        self.resize = resize
        self.branch_num = branch_num
        ...
        self.AM2 = AM(256, 128)
        self.up7 = upConv(128, 64)
        self.up7_1 = upConv(128, 64)
        self.AM3_1 = singleConv(128, 64, 3, act=True)
        self.AM3 = AM(128, 64)
        '''self.irf_gate = eca_layer(64)
        self.up8_1 = upConv(64, 32)'''
        self.AM4 = AM(64, 64)
        self.up8 = upConv(64, 32)
        self.up8_1 = upConv(64, 64)
        self.AM4_1 = nn.Sequential(
            singleConv(128, 32, 3, act=False),
            nn.GELU(),
            singleConv(32, 32, 3, act=False),
            nn.GELU()
        )
    ...
    def forward(self, x, act=True):
        ...
        if self.multitask:
            masks, _ = masks_from_regression(c10, eta=1, dim=2, device=self.device, add=False).run()
            mask_irf = 0
            for i in range(self.out_ch1-1):
                mask_irf += masks[:, i, :, :]
            #self.gussconv.weight = nn.Parameter(self.w1, requires_grad=True)
            ...
            a4 = self.AM4(up_8, c9)
            up_7_1 = self.up7_1(a2)
            merge_3_1 = torch.cat([up_7_1, a3], dim=1)
            a3_1 = self.AM3_1(torch.mul(merge_3_1, self.pool1(mask_irf)) if act else merge_3_1)
            up_8_1 = self.up8_1(a3_1)
            merge_4_1 = torch.cat([up_8_1, a4], dim=1)
            a4_1 = self.AM4_1(torch.mul(merge_4_1, mask_irf) if act else merge_4_1)
            #a4_1 = self.AM4_1(torch.mul(torch.cat([up_8_1, adp_conv1], dim=1), mask_irf) if act else torch.cat(up_8, adp_conv1))
            if self.branch_num == 4:
                up_9 = self.up9(c5)
                a5 = self.AM5(up_9, a1)
            ...
            c13 = self.conv12(c9)
            return c10, c12, c13
class tinyquadra_multi_resUnet_ly_masks_s1(nn.Module):
    def __init__(self, in_ch, out_ch1, out_ch2, out_ch3, multitask=True, device='cuda', resize=None, branch_num=4):
        super(tinyquadra_multi_resUnet_ly_masks_s1, self).__init__()
        self.multitask = multitask
        self.device = device
        self.resize = resize
        self.branch_num = branch_num
        ...
        self.AM1 = AM(512, 256)
        self.up6 = upConv(256, 128)
        self.up6_1 = upConv(256, 128)
        self.AM2_1 = singleConv(256, 128, 3, act=True)
        self.AM2 = AM(256, 128)
        self.up7 = upConv(128, 64)
        self.up7_1 = upConv(128, 64)
        self.AM3_1 = singleConv(128, 64, 3, act=True)
        ...
        self.up8 = upConv(64, 32)
        self.up8_1 = upConv(64, 64)
        self.up8_2 = upConv(64, 64)
        self.up8_3 = upConv(64, 64)
        self.AM4_1 = nn.Sequential(
            singleConv(128, 32, 3, act=False),
            nn.GELU(),
            singleConv(32, 32, 3, act=False),
    ...
    def forward(self, x, act=True):
        ...
        if self.multitask:
            ...
            a4 = self.AM4(up_8, c9)
            up_6_1 = self.up6_1(a1)
            merge_2_1 = torch.cat([up_6_1, a2], dim=1)
            a2_1 = self.AM2_1(torch.mul(merge_2_1, F.max_pool2d(mask_irf, 4, 4)) if act else merge_2_1)
            up_7_1 = self.up7_1(a2_1)
            merge_3_1 = torch.cat([up_7_1, a3], dim=1)
            a3_1 = self.AM3_1(torch.mul(merge_3_1, F.max_pool2d(mask_irf, 2, 2)) if act else merge_3_1)
            up_8_1 = self.up8_1(a3_1)
            #up_8_2 = self.up8_2(a3_1)
            #up_8_3 = self.up8_3(a3_1)
            merge_4_1 = torch.cat([up_8_1, a4], dim=1)
            a4_1 = self.AM4_1(torch.mul(merge_4_1, mask_irf) if act else merge_4_1)
            #a4_1 = self.AM4_1(torch.mul(torch.cat([up_8_1, adp_conv1], dim=1), mask_irf) if act else torch.cat(up_8, adp_conv1))
            if self.branch_num == 4:
            ...
            c13 = self.conv12(c9)
            return c10, c12, c13
class tinyquadra_multi_resUnet_ly_masks_s0(nn.Module):
    def __init__(self, in_ch, out_ch1, out_ch2, out_ch3, multitask=True, device='cuda', resize=None, branch_num=4):
        super(tinyquadra_multi_resUnet_ly_masks_s0, self).__init__()
        self.multitask = multitask
        self.device = device
        self.resize = resize
        self.branch_num = branch_num
        ...
        self.pool5 = nn.MaxPool2d(8)
        self.up5 = upConv(512, 256)
        self.up5_1 = upConv(512, 256)
        self.AM1_1 = singleConv(512, 256, 3, act=True, function='gelu')
        self.gat1 = eca_layer(512)
        self.AM1 = AM(512, 256)
        self.up6 = upConv(256, 128)
        self.up6_1 = upConv(256, 128)
        self.AM2_1 = singleConv(256, 128, 3, act=True, function='gelu')
        self.gat2 = eca_layer(256)
        self.AM2 = AM(256, 128)
        self.up7 = upConv(128, 64)
        self.up7_1 = upConv(128, 64)
        self.AM3_1 = singleConv(128, 64, 3, act=True, function='gelu')
        self.gat3 = eca_layer(128)
        self.AM3 = AM(128, 64)
        '''self.irf_gate = eca_layer(64)
        self.up8_1 = upConv(64, 32)'''
        self.AM4 = AM(64, 64)
    ...
    def forward(self, x, act=True):
        ...
        if self.multitask:
            ...
            a4 = self.AM4(up_8, c9)
            up_5_1 = self.up5_1(adp_conv5)
            merge_1_1 = torch.cat([up_5_1, a1], dim=1)
            a1_1 = self.AM1_1(torch.mul(merge_1_1, F.max_pool2d(mask_irf, 8, 8)) if act else merge_1_1)
            up_6_1 = self.up6_1(a1_1)
            merge_2_1 = torch.cat([up_6_1, a2], dim=1)
            a2_1 = self.AM2_1(torch.mul(merge_2_1, F.max_pool2d(mask_irf, 4, 4)) if act else merge_2_1)
            up_7_1 = self.up7_1(a2_1)
            merge_3_1 = torch.cat([up_7_1, a3], dim=1)
            ...
            c13 = self.conv12(c9)
            return c10, c12, c13
class tinyquadra_multi_resUnet_ly_masks_s00(nn.Module):
    def __init__(self, in_ch, out_ch1, out_ch2, out_ch3, multitask=True, device='cuda', resize=None, branch_num=4):
        super(tinyquadra_multi_resUnet_ly_masks_s00, self).__init__()
        self.multitask = multitask
        self.device = device
        self.resize = resize
        self.branch_num = branch_num
        ...
        self.up12 = upConv(64, 32)
        self.AM8 = AM(96, 64)
        self.up13 = upConv(512, 256)
        self.AM9 = AM(512, 256)
        self.up14 = upConv(256, 128)
        self.AM10 = AM(256, 128)
        self.up15 = upConv(128, 64)
        self.AM11 = AM(128, 64)
        self.up16 = upConv(64, 32)
        self.AM12 = AM(96, 32)
        self.conv10 = nn.Conv2d(32, out_ch1, 1, padding='same')
        self.conv10_1 = nn.Conv2d(64, out_ch1, 1, padding='same')
        self.conv11 = nn.Conv2d(64, out_ch2, 1, padding='same')
    ...
    def forward(self, x, act=True):
        ...
        if self.multitask:
            ...
            up_13 = self.up13(torch.mul(c5, F.max_pool2d(mask_irf,16,16)) if act else c5)
            a9 = self.AM9(up_13, torch.mul(a5, F.max_pool2d(mask_irf,8,8)) if act else a5)
            up_14 = self.up14(a9)
            a10 = self.AM10(up_14, torch.mul(a6, F.max_pool2d(mask_irf,4,4)) if act else a6)
            up_15 = self.up15(a10)
            a11 = self.AM11(up_15, torch.mul(a7, F.max_pool2d(mask_irf,2,2)) if act else a7)
            up_16 = self.up16(a11)
            a12 = self.AM12(up_16, torch.mul(a8, mask_irf) if act else a8)
            # self.featuremap1 = a8
            # print(self.featuremap1.shape)
            c13 = self.conv12(a12)
            #c13 = self.conv12(a4_1)
...
class R50_Unet(nn.Module):
    ...
    def forward(self, x, act=True):
        ...
        c10 = checkpoint.checkpoint(self.conv10, c9)
        #self.featuremap1 = c9
        if self.multitask:
            c12 = checkpoint.checkpoint(self.conv11, c9)
            c13 = checkpoint.checkpoint(self.conv12, c9)
