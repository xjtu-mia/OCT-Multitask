...
def train(root=None,
    ...
    for epoch in range(max_epoch):
        ...
        writer.add_scalar('train_loss', train_epoch_loss[6], global_step=epoch)
        writer.add_scalar('lr', optimizer.param_groups[0]['lr'], global_step=epoch)
        #writer.add_graph(model, torch.randn(1,1,448,448).to(torch.device('cuda')))
        lr_decay(optimizer, epoch, method='poly', warm_epoch=0, max_epoch=max_epoch, initial_lr=0.0001, power=1.5)
        if train_epoch_loss[6] > 100 and epoch>10:
            return val_epoch_loss[6]
        ...
        writer.add_scalar('val_loss', val_epoch_loss[6], global_step=epoch)
    ...
    with torch.no_grad():  # 不进行梯度计算和反向传播
        ...
        if multitask:
            for x, y1, y2, y3, y4 in tqdm(dataload, ncols=60):
                ...
                tmp_mad = tmp_mad + MAD(img_y_ly, img_true, softargmax=softargmax)*3.87
                tmp_rmse = tmp_rmse + RMSE(img_y_ly, img_true, softargmax=softargmax)*3.87
            ...
            #groundtruth_path = os.path.join(root, 'val', 'm1/flatted_label')
            tmp_mad = tmp_mad / len(dataload.dataset)
            tmp_rmse = tmp_rmse / len(dataload.dataset)
            print(len(dataload.dataset))
            for i, name in enumerate(name_list):
...
def run(
        ...
        log_name='duke_bs_2_ecares50unetv2_bn_adamw_acon_8layers_beta1_wIRF_tinyquadraatt_multitask_R_C_mul_ly_masks_fl_IN_test',
        ):
    ...
    save_path = os.path.join(root, "result", log_name_)
    test_path = os.path.join(root, "result", log_name_)
    ckp = os.path.join(save_path, 'weights_final.pth')
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    log_path = root + "/log/" + log_name_
    pretraining_path = os.path.join(root, "result",
                                    'duke_bs_2_ecares50unetv2_bn_adamw_acon_8layers_beta1_wIRF_tinyquadraatt_multitask_R_C_mul_ly_masks_fl_IN_test18',
                                    'weights_initial.pth')
    ...
    main(dataset=dataset,
             ...
             tr_fluid_label_fold='flatted_label_512/fluid',
             ...
             val_fluid_label_fold='flatted_label_512/fluid',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device_id', type=str, default='2', help='设备ID')
    parser.add_argument('--dataset', type=str, default='duke', help='dataset name')
    parser.add_argument('--action', type=str, default='train', help='操作类型')
    parser.add_argument('--epoch', type=int, default=65, help='max epoch for training')
    ...
    parser.add_argument('--backbone', type=str, default='resnet', help='骨干网络')
